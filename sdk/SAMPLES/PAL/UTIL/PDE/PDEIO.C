...
char *types[]      = { "Window", "IhEdit", "IhStatic", "IhButton", "IhCheck",
                       "IhRadio", "IhGroup", "IhCombo", "IhBitmap", "Other"};

char *initypes[]   = {"X", "IHEDINIT", "IHSTINIT", "IHBNINIT", "IHCKINIT",
                      "IHRAINIT","IHGRINIT","IHCBINIT","IHBMINIT","USERINIT"};

char *ininames[]   = {"X","ED","ST","BN","CK","RA","GR","CB","BM","US"};
...

{
   ...
   for(f=1;f<=pDialog->INum;f++) {
      ...
      if(pDialog->item[f]->type==9) strcpy(buffer, pDialog->item[f]->TypeName);
   ...
   do {
      ...
      if(strstr(StructDef,"DLGITEM"))  {
         ...
         for(indx=1;indx < pLoad->INum+1;indx++) {
            ...
            if(f==NUM_OF_TYPES)  /* set to default if not found */
               pLoad->item[indx]->type = 1;
         }
      }
